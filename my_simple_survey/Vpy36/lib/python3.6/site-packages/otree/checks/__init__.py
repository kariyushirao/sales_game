            ...
            if attr not in base_model_attrs[Model.__name__]:
                try:
                    ...
                    if _type in {list, dict, set}:
                        ...
                        helper.add_warning(warning, numeric_id=112)
...
def pages_function(helper: AppCheckHelper, **kwargs):
    if not helper.module_exists('views'):
        return

    views_module = helper.get_module('views')
    try:
        page_list = views_module.page_sequence
    except:
        helper.add_error(
            'views.py is missing the variable page_sequence.', numeric_id=21)
        return
    else:
